...
\endcode
	...
	*/
	//以下为吕冰添加的注释
	//Accept函数的作用是通过handler对象来处理本对象的成员。
	//handler对象一般为序列化对象
...
//以下为吕冰添加注释
//document对象用来从json text流中deserializze出dom(Object或Array)。
//
template <typename Encoding, typename Allocator = MemoryPoolAllocator<> >
...
//begin modified by lvbing 2014-11-19
		//原来的
		//GenericReader<Encoding, Allocator> reader;
		//吕冰修改为Reader使用Document的分配器
		GenericReader<Encoding, Allocator> reader(&GetAllocator());
//end modified by lvbing 2014-11-19
